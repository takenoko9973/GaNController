...
class NEALogConfig(BaseModel):
    ...
    update_date_folder: bool = Field(default=False, exclude=True)
    update_major_version: bool = Field(default=False, exclude=True)
...
class NEAControlConfig(BaseModel):
    ...
    amd_enable: bool = Field(default=False, exclude=True)  # AMDを有効化するか
...
class NEAConfig(BaseModel):
    ...
    log: NEALogConfig = Field(default_factory=NEALogConfig, exclude=True)  # ログ設定は保存しない
    execution: NEAControlConfig = Field(default_factory=NEAControlConfig)
